...
def encode_data(data: str, exist: bool):
    #converts base 3 number into base 10
    val = 0
    ...
    #mark if the word is in the list at the end
    val = (val << 1) | (1 if exist else 0)
    return val

#not needed but nice to have
def decode_data(val: int):
    # Extract the last bit for the 'exist' flag
    exist = bool(val & 1)
    #we dont need the exists flag afterwards
    val = val >> 1
